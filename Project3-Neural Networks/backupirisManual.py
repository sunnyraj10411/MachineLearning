...
class tensorFlowClassification:
    ...
    def makeNetwork(self): #we are fixing the network size to 4x4 and 4x3 for iris and 2x2 2x2 for general data
        ...
        self.layer1 = np.random.uniform(low = -1.0 ,high = 1.1, size=(   node1Size+1,node1Size)) #+1 is the bias
        ...
        self.layer2 = np.random.uniform(low=-1.0,high=1.1,size=(node1Size+1,finSize)) #+1 is the bias
    ...
    def forwardPass(self,x):
        ...
        for i in range(self.node2.shape[0]):
            ...
            self.node2[i] = self.tanh(self.node2[i])
            #print(self.node2[i])
